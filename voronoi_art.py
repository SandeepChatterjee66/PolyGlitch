...
def draw_voronoi_torch(image_path, N=3000, downscale_factor=4, lloyd_steps=2,
    ...
    for region_index in tqdm(vor.point_region, desc="Filling regions"):
        ...
        try:
            ...
            if mask.sum() == 0:
                continue
            region_color = img_tensor[:, mask].mean(dim=1)
            for c in range(3):
                canvas[c][mask] = region_color[c]
        except Exception:
            continue
...
# image_path = "Indian-Statistical-Institute-Kolkata-3.jpg"  # Replace with your image path
# draw_voronoi_gpu_fill(image_path, N=15000, save_path=image_path+"_voronoi.png")